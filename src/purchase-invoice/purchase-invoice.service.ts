...
export class PurchaseInvoiceService {
   ...
   */
  async createPurchaseInvoice(createPurchaseInvoiceDto: any, authHeader?: string): Promise<any> {
    return this.createDataGeneric('purchaseInvoices', createPurchaseInvoiceDto, authHeader);
  }
   ...
   */
  async getPurchaseInvoiceFull(id: number, authHeader?: string): Promise<any> {
    ...
    try {
      ...
      const endpoint = `${baseUrl}/api/v1/purchaseInvoices/${id}?expandLevel=full`;

      this.logger.log(`GET ${endpoint} - Detaylı satın alma faturası okunuyor`);

      return await this.httpClientService.httpGet(endpoint, token);

    } catch (error) {
      this.logger.error(`Detaylı satın alma faturası okuma hatası (ID: ${id}):`, error.message);
      throw new HttpException(
        `Detaylı satın alma faturası okunamadı: ${error.message}`,
  ...
  /**
   * Satın alma faturaları API URL'ini döndür
   */
